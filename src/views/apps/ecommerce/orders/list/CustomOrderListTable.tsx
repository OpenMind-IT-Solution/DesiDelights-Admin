...
const OrderListTable = ({ orderData }: { orderData?: OrderType[] }) => {
  ...
  const handleDownloadSelected = (
    selectedUsers: ECommerceOrderTypeWithAction[],
    allUsers: ECommerceOrderTypeWithAction[]
  ) => {
    ...
    const rows = usersToExport.map(user =>
      headers.map(header => escapeCSV(user[header as keyof ECommerceOrderTypeWithAction]))
    )
