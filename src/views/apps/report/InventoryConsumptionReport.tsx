...
const InventoryConsumptionReport = () => {
  const { data, loading, error, applyRange, fetch } = useReport<InventoryConsumptionData>({ endpoint: reportEndpoints.inventoryConsumption })
  ...
  return (
    <>
      ...
      <ReportTable<InventoryConsumptionItem>
        ...
        loading={loading}
        error={error}
        emptyMessage='No inventory consumption data found'
      />
    </>
  )
}
